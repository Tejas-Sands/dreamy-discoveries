 ...
 *   - sprinkles peek-a-boo gags into long calm scenes
...
import { validateCharacterRecipe, validateBackgroundRecipe } from "./recipes.mjs";
import { hintCharacter, hintBackground } from "./hints.mjs";
import { saveRecipe } from "./library.mjs";

const SPECIES_EMOJI = CHARACTER_EMOJI;

/** LLM-proposed recipes: validate, save to the library, make them usable right away */
function adoptNewRecipes(script) {
  ...
  for (const item of Array.isArray(script.newCharacters) ? script.newCharacters : []) {
    ...
    const recipe = validateCharacterRecipe(item?.recipe ?? item, name);
    if (!recipe || CHARACTERS.includes(recipe.name)) continue;
    saveRecipe("characters", recipe.name, recipe);
    CHARACTERS.push(recipe.name);
    SPECIES_EMOJI[recipe.name] = recipe.emoji;
    DEFAULT_NAMES[recipe.name] = recipe.words.name;
    adopted.push(`character:${recipe.name}`);
  }
...
const GAG_ORDER = ["monkey", "frog", "duck", "cat", "pig", "dog", "bunny", "bear"];
...
function normalizeLine(raw) {
  ...
  const line = { ...raw, text: raw.text.trim() };
...
function normalizeScene(raw, main) {
  ...
  scene.lines = (Array.isArray(scene.lines) ? scene.lines : []).map(normalizeLine).filter(Boolean);
...
function otherSpecies(exclude) {
  for (const k of GAG_ORDER) if (!exclude.includes(k)) return k;
  return "monkey";
}
...
export function directScript(input, opts = {}) {
  ...
  script.scenes.forEach((scene, si) => {
    ...
    // a friend mentioned by species name in a story gets to stand on the right
    if (script.type === "story" && scene.secondCharacter === undefined && scene.character !== "none") {
      const joined = scene.lines.map((l) => l.text).join(" ");
      const mentioned = CHARACTERS.find(
    ...
    if (!scene.transition) {
      scene.transition =
        si === 0 ? "pop"
        : scene.kind === "chorus" || scene.kind === "moral" ? "pop"
        : isQuestion ? "iris"
        : ["slide", "wipe", "fade"][transitionAlt++ % 3];
    }
    if (!scene.camera) {
      scene.camera = scene.energy === "upbeat" ? "still" : ["zoom-in", "pan", "zoom-out"][cameraAlt++ % 3];
      if (isQuestion) scene.camera = "zoom-in";
    }

    // peek-a-boo gag in long calm scenes (set "gag": false to switch it off for a scene)
    if (scene.gag && typeof scene.gag === "object") {
      ...
      const eligible =
        scene.energy === "calm" && scene.lines.length >= 3 && !scene.secondCharacter &&
        scene.character !== "none" && si - lastGagScene >= 4 && scene.kind !== "lesson" && scene.kind !== "moral";
      if (eligible) {
        scene.gag = { kind: "peek", character: otherSpecies([scene.character, main.kind]), atSec: 2.2 };
        lastGagScene = si;
      } else {
        scene.gag = null;
      }
